...
it('starts an active exam attempt and allows submission', async () => {
    ...
    const startResponse = await (0, supertest_1.default)(app_1.app)
        .post(`/api/exams/${examId}/start`)
        .set('Cookie', studentCookie)
        .expect(201);
    expect(startResponse.body.attempt.status).toEqual('in_progress');
    const repeatStartResponse = await (0, supertest_1.default)(app_1.app)
        .post(`/api/exams/${examId}/start`)
        .set('Cookie', studentCookie)
        .expect(200);
...
it('expires an exam and blocks expired attempts from submission', async () => {
    ...
    await (0, supertest_1.default)(app_1.app)
        ...
        .set('Cookie', studentCookie)
        .expect(201);
    const expireResponse = await (0, supertest_1.default)(app_1.app)
