...
async function createCourse(cookie, payload) {
...
it('allows student to list all courses in their institution', async () => {
    ...
    expect(response.body.courses.map((course) => course.code).sort()).toEqual(['CSC-401', 'CSC-402']);
    expect(response.body.courses.every((course) => course.isRegistered === false)).toBe(true);
});
it('allows student to register for a course and returns idempotent response on repeat', async () => {
    ...
    const firstRegistration = await (0, supertest_1.default)(app_1.app)
    ...
    const secondRegistration = await (0, supertest_1.default)(app_1.app)
        .post(`/api/exams/courses/${createdCourse.body.course.id}/register`)
        .set('Cookie', studentCookie)
        .expect(200);
